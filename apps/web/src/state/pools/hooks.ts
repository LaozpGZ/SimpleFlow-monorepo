...
import { getSourceChain, isIfoSupported } from '@simpleflow/ifos'
import { getLivePoolsConfig } from '@simpleflow/pools'
  ...
  return farmsConfig
    .filter(
      ({ token, pid, quoteToken }) =>
        pid !== 0 &&
        ((token.symbol === 'SDX' && quoteToken.symbol === 'WBNB') ||
