...
const picksConfigAtom = atom(async () => {
  ...
  const urlPreview = `https://proofs.simpleflow.finance/picks/today-preview.json?t=${time}`
  const showPreviewVersion = process.env.NEXT_PUBLIC_VERCEL_ENV !== 'production'
  const url = showPreviewVersion ? urlPreview : `https://proofs.simpleflow.finance/picks/today.json?t=${time}`
  try {
