...
import { Fragment, Suspense, ReactNode } from 'react'
...
const _LoadVConsole = dynamic(() => import('components/vConsole'), { ssr: false })
...
export function AppContainer({ children, store, dehydratedState, isVite = false }: AppContainerProps) {
  const [_isOpen, _setIsOpen] = useAtom(walletModalVisibleAtom)
  ...
  return (
    <>
      ...
      <DefaultSeo {...SEO} />
      {/* <LoadVConsole /> */}
      <Providers store={store} dehydratedState={dehydratedState}>
        <ProductionErrorBoundary>
          ...
          <PersistGate loading={null} persistor={persistor}>
            <Updaters />
            <AppContent isVite={isVite}>{children}</AppContent>
          </PersistGate>
        </ProductionErrorBoundary>
      </Providers>
      <Script
...
function AppContent({ children, isVite = false }: AppContentProps) {
  const [isOpen, setIsOpen] = useAtom(walletModalVisibleAtom)

  return (
    <Suspense>
      {isVite ? (
        // For Vite, render children directly without menu/layout
        <>
          {children}
        ...
        // For Next.js, use the full layout with menu
        <>
          <SharedComponentWithOutMenu>
            <Menu>
              {children}
