export const PCS_LIMIT_ORDER_POOLS_URL = 'https://proofs.simpleflow.finance/cms-config/limit-order-pools-config.json'

export const PCS_LIMIT_ORDER_HISTORY_URL = 'https://limit-order-api.simpleflow.finance/api/limit-order/history'

export const DEFAULT_PERCENTAGE_MAP = {
  1: '1',
  5: '5',
  10: '10',
}

export const ORDERS_PER_PAGE = 4

// From contract documentation, we can have up to 50 pending orders
export const MAX_PENDING_ORDERS = 50

export const MIN_BNB_VALUE = 0.05
export const MIN_USD_VALUE = 50 // 50 USD
